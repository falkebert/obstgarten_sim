...
while game_number <= total_games-1:
  ...
  while not is_game_over:
    ...
    def choose_biggest_tree(tree_lottery_winner):
      max_value = max(red_tree, blue_tree, green_tree, yellow_tree)
      tree_lottery = []
      if red_tree == max_value:
          tree_lottery.append("red")
      if blue_tree == max_value:
          tree_lottery.append("blue")
      if green_tree == max_value:
          tree_lottery.append("green")
      if yellow_tree == max_value:
          tree_lottery.append("yellow")
      tree_lottery_winner = random.choice(tree_lottery)
      print("chooses " + str(tree_lottery_winner))
      return tree_lottery_winner
    
    if dice_result == "basket":
      who_won_lottery = choose_biggest_tree(who_won_lottery)
